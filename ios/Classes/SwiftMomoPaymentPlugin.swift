...
public class SwiftMomoPaymentPlugin: NSObject, FlutterPlugin {
    final var METHODREQUESTPAYMENT = "requestPayment"
    var momoFlutterResult: FlutterResult? = nil
    
    public static func register(with registrar: FlutterPluginRegistrar) {
        let channel = FlutterMethodChannel(name: "com.etech.momo_payment/momo_payment", binaryMessenger: registrar.messenger())
        let instance = SwiftMomoPaymentPlugin()
        registrar.addMethodCallDelegate(instance, channel: channel)
    }

    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        if(call.method == METHODREQUESTPAYMENT) {
